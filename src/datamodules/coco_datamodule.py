...
class COCODataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transfroms is not None:
            transformed = self.transfroms(
                image=np.array(img), bboxes=target["boxes"], class_labels=target["labels"]
            )
...
class COCODataModule(LightningDataModule):
    def __init__(
        ...
        data_split: Tuple[int, int, int] = [113_287, 5000, 0],
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if self.hparams.data_root_val != None and self.hparams.ann_file_val != None:
            self.train_dataset = COCODataset(
                self.hparams.data_root_train,
                self.hparams.ann_file_train,
                self.hparams.train_transforms,
            )
            self.val_dataset = COCODataset(
                self.hparams.data_root_val, self.hparams.ann_file_val, self.hparams.val_transforms
            )
    ...
    def default_transforms(self):
        default_t = A.Compose(
            ...
            bbox_params=A.BboxParams(
                format="pascal_voc", label_fields=["class_labels"], min_visibility=0.4, min_area=5
            ),
        )
        return default_t
